...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { threshold: 0.35, rootMargin: "2000px 0px 0px 0px" } // Trigger when 10% of the section is visible
    );
          ...
          <div className="absolute top-0 w-full">
            <div className="relative z-20 section-width max-w-screen-2xl mx-auto top-8 max-2xl:px-36 max-md:px-14 max-sm:px-5">
              ...
              >
                <ul
                  ref={gradeListRef}
                  className="flex justify-center flex-row-reverse gap-10  text-gray-700 font-semibold [&_button]:cursor-pointer overflow-x-auto px-10 w-full"
                >
          ...
          <ol className="overflow-hidden space-y-6  w-full pl-6 max-sm:p-4">
            {stepperData.map((step, index) => (
              <li
                ...
                className={`relative flex-1 transition-all duration-500 ease-in-out ${index !== stepperData.length - 1
                  ? `after:content-[''] after:h-full after:inline-block after:absolute max-sm:after:hidden  after:-bottom-14 after:right-[1.6rem] transition-all ${activeStep > index + 1 ? "after:w-2 " : "after:w-0.5"
                  }`
                  : ""
                  } ${activeStep >= index + 1
                    ? "after:bg-[#FEC23B]"
              ...
              >
                <div className="flex flex-row-reverse font-medium w-full gap-4">
                  <span
                    className={`w-10 h-10 max-sm:hidden rounded-full flex justify-center items-center mr-3 text-sm mt-4 transition-all duration-500 ease-in-out ${activeStep >= index + 1
                  ...
                  <div
                    className={`flex items-center max-sm:items-end max-sm:flex-col-reverse justify-end gap-4  bg-[#F6F6F6] p-6 transition-all pl-20 max-lg:pl-6 w-full rounded-[16px] duration-500 ease-in-out ${activeStep === index + 1
                  ...
                  >
                    <div className="flex flex-col">
                      <h4
                        className={`text-lg text-[15px] mb-2 text-end font-bold max-sm:text-sm ${activeStep === index + 1 ? "" : "text-gray-900"
                          }`}
                      >
                        {step.title}
                      </h4>
